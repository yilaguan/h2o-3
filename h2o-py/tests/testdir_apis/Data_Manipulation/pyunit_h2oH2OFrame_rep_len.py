...
def h2o_H2OFrame_rep_len():
    ...
    allframe = h2oframe.rep_len(length_out=col_num+length_out_c)
    assert_is_type(allframe, H2OFrame)      # check return type
        # check shape
    assert allframe.shape==(row_num, col_num+length_out_c), "h2o.H2OFrame.rep_len() command is not working."
        # check values
    pyunit_utils.compare_frames(h2oframe[range(0, length_out_c)], allframe[range(col_num, col_num+length_out_c)],
                                row_num, tol_time=0, tol_numeric=1e-6, strict=False, compare_NA=True)
