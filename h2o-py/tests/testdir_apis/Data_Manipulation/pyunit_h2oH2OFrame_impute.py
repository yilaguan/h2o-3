...
def h2o_H2OFrame_impute():
    ...
    """
    h2o.connect()
    python_lists = np.random.randint(-5,5, (101,3))
    ...
    assert abs(h2oframe[row_ind_median, 0]-impute_median) < 1e-6, "h2o.H2OFrame.impute() command is not working."
    assert abs(h2oframe[row_ind_mode, 0]-impute_mode) < 1e-6, "h2o.H2OFrame.impute() command is not working."
